...
from NoTmeowL import DEcwHisPErMsG_pb2 , MajoRLoGinrEs_pb2 , PorTs_pb2 , MajoRLoGinrEq_pb2 , sQ_pb2 , Team_msg_pb2, RemoveFriend_Req_pb2, GetFriend_Res_pb2, spam_request_pb2, devxt_count_pb2, dev_generator_pb2, kyro_title_pb2, room_join_pb2
...
def get_player_info(uid):
    try:
        url = f"https://your-api.vercel.app/info/get?uid={uid}"
...
async def handle_reject_completion(spam_task, target_uid, sender_uid, chat_id, chat_type, key, iv):
    ...
    try:
        ...
        if spam_count >= 150:
            ...
            completion_msg = f"[B][C][FFD700]⚡⚠️ Reject Spam Partially Completed for ID {xMsGFixinG(target_uid)}\n⚠️ Total packets sent: {spam_count * 2}\n"
...
async def black666(client_id, key, iv):
    banner_text = "[FF0000][B][C] ERROR , WELCOME TO [FFFFFF]NoTmeowL [00FF00]___ NoTmeowL____ BOT ! \n[FFD700]⚡NEW VERSION NEW FUNCTION !\n[FF0000]TELEGRAM : @MG24_GAMER\n\n"     
    fields = {
        1: 5,
        2: {
            1: int(client_id),
...
async def handle_msg_spam_completion(spam_task, message_text, times, sender_uid, chat_id, chat_type, key, iv):
    ...
    try:
        ...
        if actual_times >= times:
            ...
            completion_msg = f"[B][C][FFD700]⚡⚠️ MESSAGE SPAM PARTIALLY COMPLETED!\n"
            completion_msg += f"[FFFFFF]📝 Message: {message_text}\n"
            completion_msg += f"[FFFFFF]📊 Requested: {times} times\n"
            completion_msg += f"[FFFFFF]⚠️ Sent: {actual_times} times\n"
            completion_msg += f"[FFD700]⚡↯ Success rate: {(actual_times/times)*100:.1f}%\n"
            completion_msg += f"[FFFFFF]💬 Check squad chat to see messages!\n"
        else:
...
def send_tiktok_info(username):
    ...
    try:
        response = requests.get(
            f"https://your-api.vercel.app/tt-info/tiktok?username={username}",
            timeout=15
        )
        ...
        return f"""
[B][C][FFD700]╭[FFD700]⚡─[FFD700]⚡╮[FFFFFF]
[C][B][00bFFF]│[00bFFF]ꚠ[00bFFF] │[FFFFFF]║[00bFFF]TIKTOK INFO[FFFFFF]║
[C][B][FF00FF]╰[FFD700]⚡─[FFD700]⚡╯[FFFFFF]
[C][B][FF00FF]━━━━━━━━━━━
[C][B][FFFFFF]Fullname   : [FFD700]⚡{full_name}
[C][B][FFFFFF]Username   : [FFD700]⚡{username_}
...
[C][B][FFFFFF]Private    : [FFD700]⚡{private_status}
[C][B][00FFFF]━━━━━━━━━━━
"""
...
def get_youtube_info(channel_name):
    try:
        response_json = requests.get(
            f"https://your-api.vercel.app/yt-info/yt?channel={channel_name.lstrip('@')}",
            timeout=15
        ).json()
...
[C][B][FF00FF]━━━━━━━━━━━
[C][B][FFFFFF]Channel Name : [FFD700]⚡{response_json.get('channel_title', 'Unknown')}
[C][B][FFFFFF]Channel ID    : [FFD700]⚡{response_json.get('channel_id', 'Unknown')}
...
[C][FFD700]UID         : [00FFAA]{xMsGFixinG(captain.get('accountId'))}
...
[C][FFAA00]Create Date   : [00FF00]{ff_num(human_time(acc.get('AccountCreateTime', '0'))[:16])}
[C][FFAA00]Last Login    : [00FF00]{ff_num(human_time(acc.get('AccountLastLogin', '0'))[:16])}
...
def get_math_result(input_expr):
    ...
    url = f"https://your-api.vercel.app/math/math?expression={encoded_expr}"
...
def send_likes(uid):
    try:
        likes_api_response = requests.get(
             f"https://mg24-gamer-king-like.onrender.com/like?uid={uid}&server_name=bd",
             timeout=15
             )
...
[C][B][11EAFD]‎━━━━━━━━━━━━
[C][B][FFB300]Subscribe: [FFFFFF]NoTmeowL [00FF00]!!
"""
...
def fake_likes(uid):
    try:
        likes_api_response = requests.get(
             f"https://your-api.vercel.app/fake_like/like?uid={uid}&server_name=bd",
             timeout=15
             )
...
def talk_with_ai(question):
    url = f"https://chat-nm.vercel.app/ai/ask?key=MG24_GAMER&question={question}"
    res = requests.get(url)
    if res.status_code == 200:
        data = res.json()
...
def get_random_color():
    colors = [
        "[FF0000]", "[00FF00]", "[0000FF]", "[FFD700]⚡", "[FF00FF]", "[00FFFF]", "[FFFFFF]", "[FFA500]",
...
"""
            ...
            waiting_msg = f"""[B][C][FFD700]⚡⏱️ MATCH IN PROGRESS...
...
[FF0000]━[00FF00]━[0000FF]━[FFD700]⚡━[FF00FF]━[00FFFF]━[FFA500]━[FF1493]━[00FF7F]━[FFD700]━[00CED1]━[9400D3]━[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
...
"""
    ...
    try:
        for index, title_id in enumerate(all_titles):
            ...
            progress_msg = f"""[B][C][FFD700]⚡📤 SENDING TITLE {title_number}/{total_titles}
...
async def handle_room_command(inPuTMsG, uid, chat_id, key, iv, region, chat_type):
    ...
    try:
        ...
        if not player_status or "IN ROOM" not in player_status:
            info_msg = f"""[B][C][FFD700]⚡📊 STATUS: {player_status or 'UNKNOWN'}
...
"""
        ...
        if accounts:
            preview_msg = f"""[B][C][FFD700]⚡🔍 FIRST ACCOUNT PREVIEW:
              ...
              # =================== EMOTE DETECTION ONLY ===================
                ...
                if insquad == True:
                    try:
                        ...
                        message = """[B][C][FF0000]█▓▒░░ WELCOME TO NoTmeowL BOT ░░▒▓█
[00FF00]»»————-　★　————-««
[FFFFFF]⚡ BOT POWERED BY:[FF0000] NoTmeowL
[FFFFFF]🎯 STATUS: [00FF00]ACTIVE 24/7
[FFFFFF]📱 CONTACT: [FFD700]⚡@NoTmeowl.
[00FF00]»»————-　★　————-««
                                    ...
                                    try:
                                        ...
                                        if result.get("success"):
                                            ...
                                            if attempt < max_retries - 1:
                                                ...
                                                progress_msg = f"[B][C][FFD700]⚡🔄 Retrying... (Attempt {attempt + 2}/{max_retries})\n"
...
# GUILD INFORMATION FF
                        ...
                        if inPuTMsG.strip() == '/wldisable':
                            ...
                            WHITELIST_ONLY = False
                            msg = f"[B][C][FFD700]⚡⚠️ Whitelist-only mode DISABLED!\n🤖 Bot will accept invites from anyone\n"
                            await safe_send_message(response.Data.chat_type, msg, uid, chat_id, key, iv)
...
"""
                                ...
                                if accounts:
                                    ...
                                    recent_msg = "[B][C][FFD700]⚡📋 RECENT 5 ACCOUNTS:\n"
...
[FFD700]✨ Name on instagram : [FFFFFF]Yur_Pushpa❤️  
[FF0000]╚══════════╝
...
# GLOBAL BLOCKED NAMES
                        BLOCKED_NAMES = ["maruf", "nm", "mg-king", "mg_king", "m", "mg king"]  # Protected names
                            ...
                            try:
                                ...
                                for attempt in range(max_retries):
                                    ...
                                    if cache_data:
                                        ...
                                        # Build response
                                        status_msg = f"[B][C][FFD700]⚡📊 PLAYER STATUS\n"
                                        status_msg += f"────────────────\n"
...
#==================≈===========  /HELP MENU COMMANDS ========================================
    ...
    # ───── Commands 1-10 ─────
                            help_1 = '''[B][FFD700]═══⚓︎ NoTmeowl'S BOT | HELP 1/6 ⚓︎═══
[b][FFD700]⚡ increase Group Limit 
[FF6600]🤫/3 🤫/5 🤫/6🤫
[FFD700]⚡  Get Player Info 
[FF6600]/info [uid]
[FFD700]⚡ Invite Any Player
[FF6600]/inv [uid]
[FFD700]⚡ Spam Msg In Chat
[FF6600]/ms [msg]
[FFD700]⚡  Unlock 5v5 Squad 
[FF6600]/snd [uid]
[FFD700]⚡ Get Admin info
[FF6600]/admin
[FFD700]⚡  Get Bot Join Team 
[FF6600]/join [TeamCode]
[FFD700]⚡ Bot Get Leave Group 
[FF6600]/exit
[FFD700]━━━━━━━━━━━━'''
    ...
    # ───── Commands 11-20 ─────
                            help_2 = '''[C][B][FFD700]═══⚓︎ NoTmeowl'S BOT | HELP 2/6 ⚓︎═══[00FF00][B]
[b][FFD700]⚡ Spam Req Player 
[FF6600]/spam [uid]
[FFD700]⚡  Kick Player in group 
[FF6600]/kick [uid]
[FFD700]⚡  Lag Team
[FF6600]/lag [TeamCode]
[FFD700]⚡  Join Req Spam 
[FF6600]/spam_join [uid]
[FFD700]⚡  Attack on Group 
[FF6600]/attack [TeamCode]
[FFD700]⚡  Bypass Rank Requirement
[FF6600]/start [TeamCode]
[FFD700]⚡  Set Times Emote 
[FF6600]/p [uid] [emote] [count]
[FFD700]━━━━━━━━━━━━
    '''
    ...
    # ───── Commands 21-30 ─────
                            help_3 = '''[C][B][FFD700]═══⚓︎ NoTmeowl'S BOT | HELP 3/6 ⚓︎═══[00FF00][B]
[FFD700]⚡ Show Bundle List
[FF6600]/bundle
[FFD700]⚡  EVO Emote Cycle 
[FF6600]/evos [uid]
[FFD700]⚡  Stop Evo Cycle 
[FF6600]/sevos
[FFD700]⚡  Emote Any Play 
[FF6600]/play [uid] [1-410]
[FFD700]⚡ Play Emote With E/id
[FF6600]/emote [U1] [T-C] [E-Num]
[FFD700]⚡  EVO Emote With Id
[FF6600]/evo [U1] [U2] [E-Num]
[FFD700]⚡  Play Emote With Id
[FF6600]/e [UID1] [UID2] [E-ID]
[FFD700]⚡  Fast Play Emote
[FF6600]/fast [U1] [U2] [E-ID]
[FFD700]━━━━━━━━━━━━'''
                            await safe_send_message(response.Data.chat_type, help_3, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

    # ───── Commands 31-40 ─────
                            help_4 = '''[C][B][FFD700]═══⚓︎ NoTmeowl'S BOT | HELP 4/6 ⚓︎═══[00FF00][B]
[c][b][FFD700]⚡ All Badges Invite
[FF6600]/s1, /s2, /s3, /s4, /s5
[FFD700]⚡  All Badge Join Spam
[FF6600]/spam [uid]
[FFD700]⚡  Spam Gali To Friend 
[FF6600]/gali [name]
[FFD700]⚡  Equip Any Bundle
[FF6600]/bundle [name]
[FFD700]⚡  Chat With chatGpt
[FF6600]? [question]
[FFD700]━━━━━━━━━━━━'''
    ...
    # ───── Commands 41-50 ─────
                            help_5 = '''[C][B][FFD700]═══⚓︎ NoTmeowl'S BOT | HELP 5/6 ⚓︎═══[00FF00][B]
[FFD700]⚡  Set Bundle For Bot
[FF6600]/bundle [name]
[FFD700]⚡  Lag any Team  
[FF6600]/lag [TeamCode]
[FFD700]⚡  Attack on Team
[FF6600]/attack [TeamCode]
[FFD700]⚡  L-w Level Up Bot
[FF6600]/lw [TeamCode]
 [FFD700]⚡  Evo Cycle Self [FF6600]@evos
[FFD700]⚡  Stop Evo Cycle [FF6600]@sevos
[FFD700]━━━━━━━━━━━━'''
                            await safe_send_message(response.Data.chat_type, help_5, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

    # ───── Commands 51-62 ─────
                            help_6 = '''[C][B][FFD700]═══⚓︎ NoTmeowl'S BOT | HELP 6/6 ⚓︎═══[00FF00][B]
[c][b][FFD700]⚡  Bundle Lag Spam 
[FF6600]/bb_lag [TeamCode]
[FFD700]⚡  Spam Invite 
[FF6600]/spm_inv [uid]
[FFD700]⚡  Spam Join Requests [FF6600]/spam_join [uid]
[FFD700]⚡  Play Emote Self 
[FF6600]/e [emote]
[FFD700]⚡  Play Emote Player [FF6600]/e [uid] [emote]
[FFD700]⚡  Play Emote Without Bot [FF6600]/me [TeamCode] [emote]
[FFD700]⚡  Player Emote Without Bot [FF6600]/me [tc] [uid] [emote]
[FFD700]━━━━━━━━━━━━'''
